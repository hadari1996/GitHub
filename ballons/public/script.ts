...
let background_music= document.querySelectorAll("audio")[1]!
let life= document.createElement("label")
...
let board_game=document.querySelector(".background")!;
balloon.setAttribute("src", "balloon.png");
div_img.appendChild(balloon)
board_game?.appendChild(div_img);
...
let kind_balloon="classic";
let id:any;
let sum_score_label= document.createElement("label");
let sum_score_div= document.createElement("div");
...
function create_balloon(kind_balloon:string){
    ...
    score.textContent=`score: ${score_points}`;
    life.textContent=`life: ${count_loose}`;
    let b_x_place:Number= Math.round(Math.random()* 900) + 100;
...
let tune= document.querySelectorAll("audio")[0]!
let fly_tune= document.querySelectorAll("audio")[2]!
    ...
    if(count_loose==0)
        {
            ...
            let board_game=document.querySelector(".background")!;
            board_game.appendChild(link_btn);
